---
title: Configuration
description: Everything glacis.yaml controls — policy, controls, sampling, judges, attestation mode and evidence storage.
sidebar:
  order: 8
---
...
controls:
  ...
  input:
    ...
    topic:
      enabled: true
      allowed_topics: ["healthcare", "medical"] # Must match at least one
      blocked_topics: []                       # Must not match any
      if_detected: "block"
    prompt_security:
      enabled: true
      ...
      if_detected: "block"
    grounding:
      enabled: false                           # Stub: always passes
      threshold: 0.5
      if_detected: "flag"
  output:
    pii_phi:
...
attestation:
  offline: true           # true = offline (local signing), false = online (server signs)
  service_id: "my-service"

# --- Evidence storage ---
evidence_storage:
  backend: "sqlite"       # "sqlite" or "json"
  # path: "/var/lib/glacis/glacis.db"
  #   Omit for the default (~/.glacis/glacis.db for sqlite, ~/.glacis for json).
  #   An explicit path is used literally — `~` is NOT expanded to your home
  #   directory. Write an absolute path, or leave it out.
```
...
Both stages support the same seven built-in controls: `pii_phi`, `word_filter`, `jailbreak`, `content_safety`, `topic`, `prompt_security`, and `grounding`.
...
**`content_safety` -- Toxicity Detection**

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | `bool` | `false` | Enable content safety scanning |
| `model` | `str` | `"toxic-bert"` | Detection model (currently: `"toxic-bert"`) |
| `threshold` | `float` | `0.5` | Score threshold (0.0 to 1.0) |
| `categories` | `list[str]` | `[]` | Toxicity categories to check. Empty = all |
| `if_detected` | `"forward"` \| `"flag"` \| `"block"` | `"flag"` | Action when harmful content is detected |

<Aside type="caution">
  Content safety needs `transformers` (and a model download at first use).

  The SDK's own `ImportError` tells you to run `pip install glacis[content-safety]`
  — **that extra does not exist in 0.8.0**. The published extras are `openai`,
  `anthropic`, `gemini`, `litellm`, `redaction`, `jailbreak`, `controls`, `dev`
  and `all`. Install `transformers` directly, or take it as part of
  `glacis[jailbreak]`.
</Aside>

**`topic` -- Topic Enforcement**

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | `bool` | `false` | Enable topic enforcement |
| `allowed_topics` | `list[str]` | `[]` | Allowed keywords (text must match at least one) |
| `blocked_topics` | `list[str]` | `[]` | Blocked keywords (any match = flagged) |
| `if_detected` | `"forward"` \| `"flag"` \| `"block"` | `"flag"` | Action when topic violation is detected |

**`prompt_security` -- Prompt Extraction Detection**

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | `bool` | `false` | Enable prompt security scanning |
| `patterns` | `list[str]` | `[]` | Additional regex patterns (added to built-in set) |
| `if_detected` | `"forward"` \| `"flag"` \| `"block"` | `"block"` | Action when extraction attempt is detected |

**`grounding` -- Grounding Stub**

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | `bool` | `false` | Enable grounding control (stub) |
| `threshold` | `float` | `0.5` | Threshold (for custom implementations extending the stub) |
| `if_detected` | `"forward"` \| `"flag"` \| `"block"` | `"flag"` | Action when grounding issue is detected |

<Aside type="note">
  The built-in grounding control is a pass-through stub. For real grounding validation, use the `custom` controls section with a `BaseControl` subclass that accepts reference text.
</Aside>
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `offline` | `bool` | `true` | `true` = offline mode (local Ed25519 signing), `false` = online mode (the server signs; [not the same as witnessed](/connect/offline-vs-witnessed/)) |
...
| `path` | `str \| null` | `null` (defaults to `~/.glacis/glacis.db` for SQLite, `~/.glacis` for JSON) | For SQLite: full `.db` file path. For JSON: directory containing `.jsonl` files. **`~` is not expanded** — give an absolute path or omit the key. |
...
client = attested_openai(
    config="./glacis.yaml",
    # attestation.offline defaults to true, so a seed is required.
...
<Aside type="caution" title="The config file cannot supply the signing key">
  `attestation.offline` defaults to **true** — both in the SDK's built-in
  defaults and in a `glacis.yaml` that does not say otherwise. Offline mode
  needs a 32-byte seed, and `glacis.yaml` has no field for one, on purpose: a
  signing key does not belong in a file you commit.

  So a wrapper constructed with `config=` and nothing else raises at
  construction time:

  ```
  ValueError: signing_seed is required for offline mode
  ```

  Pass `signing_seed=` from your environment or secrets manager, or set
  `attestation.offline: false` in the YAML and pass `glacis_api_key=`.
</Aside>
