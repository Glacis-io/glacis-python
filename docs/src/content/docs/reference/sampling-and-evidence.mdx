...
- **L1** retains input and output locally for audit, **through the provider wrappers**, which call `store_evidence()`. What they store is a projection of the request and response, not everything the provider saw. The data never leaves your environment. A direct `attest()` call does not write full evidence: offline it stores 100-character previews, and the online path writes nothing locally at all — but an online L1/L2 attestation comes back with your **raw** input and output attached to `attestation.evidence.data` in memory, so serialising that object serialises the prompt. See [what is retained locally](/connect/offline-vs-witnessed/#what-is-retained-locally).
