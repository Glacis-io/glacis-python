---
title: Controls
description: Input and output scanning for PII/PHI, jailbreak attempts, toxicity, topic drift and prohibited terms — recorded in the receipt.
---
...
<Aside type="caution">
  Content safety needs `transformers`, plus a model download on first use.

  The SDK's own `ImportError` tells you to run `pip install glacis[content-safety]`
  — **that extra does not exist in 0.8.0**. Install `transformers` directly, or
  take it as part of `glacis[jailbreak]`. (Checked against `pyproject.toml` by
  `docs/scripts/verify-doc-snippets.py`.)
</Aside>
...
| `version` | `str` | Stamped from an internal constant. In 0.8.0 that constant still reads `"0.7.0"` — it is **not** the installed package version. Do not use it to identify the SDK build. |
...
- [`demos/custom_control_demo.ipynb`](https://github.com/Glacis-io/glacis-python/blob/main/demos/custom_control_demo.ipynb) -- step-by-step notebook building a custom control from scratch
- [Configuration](/connect/configuration/) -- full `glacis.yaml` reference
- [API Reference](/reference/api/) -- `ControlPlaneResults`, `ControlExecution` models
- [Sampling & Evidence](/reference/sampling-and-evidence/) -- how controls interact with L1/L2 sampling
