...
<Aside type="danger" title="Up to 0.8.0 the built-in backends dropped it">
  ...
  `Glacis.verify()` did not catch it either, because the offline branch only
  compared the locally derived public key and never checked a signature. As of
  0.8.1 it does check the signature, so a receipt that lost its control-plane
  content now fails there too.
