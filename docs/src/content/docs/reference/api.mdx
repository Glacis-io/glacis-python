...
attestations, see the caveat below — 0.8.0 does **not** check the signature.
...
<Aside type="caution" title="Offline `verify()` is not a signature check in 0.8.0">
  The offline branch derives a public key from the client's own `signing_seed`
  and compares it to `attestation.public_key`. If they match it returns
  `valid=True` — **without looking at `signature` at all**. Replace the
  signature with zeroes and it still passes. If the client has no seed (a
  receipt loaded from local storage), `signature_valid` is hardcoded `True`.

  `glacis.crypto.Ed25519Runtime` has `sign()` and `get_public_key_hex()` but no
  `verify()`, so the SDK cannot perform this check today.

  For a real verification, use the PyNaCl routine in
  [Verify from Python](/verify/cli/#verify-a-receipt-yourself). This behaviour
  is pinned by `docs/scripts/verify-doc-snippets.py` so the page cannot go
  stale silently.

  0.8.1 (unpublished) adds exactly one case where the offline branch returns
  `valid=False`: a receipt reconstructed from a store that could not return its
  signed `control_plane_results`. That is not a signature check either — it is
  the SDK refusing to report a verdict on a payload it knows cannot be rebuilt.
  The `error` field carries the reason.
</Aside>
...
| `signature_valid` | `bool` | **Not** an Ed25519 check in 0.8.0 — see the caveat under [`verify()`](#glacisverify) |
