---
title: API reference
description: Every public class, method and model in the glacis Python SDK 0.8.0.
---

import { Aside } from '@astrojs/starlight/components';

The full public surface of `glacis` 0.8.0. If you are getting started, read
[Connect](/connect/) first — this page assumes you know what an attestation is.
...
Create an attestation for an AI operation. `input` and `output` are hashed locally using the SDK's canonical JSON + SHA-256 (see [the canonicalisation actually used](/connect/quickstart/#the-canonicalisation-actually-used) -- the docstrings call it RFC 8785, and it is not quite). Only their hash is sent to the server -- those two payloads never leave your infrastructure.

`control_plane_results` is the exception: online mode sends it **verbatim** in the request body, uninspected and unhashed, alongside `cpr_hash`. Anything you put in it leaves the machine. See [the data boundary](/connect/offline-vs-witnessed/#what-is-actually-sent).
...
Verify an attestation. Given an **id string** this is a lookup: an `oatt_` id is
read back from local storage and its signature checked, anything else is
verified by the server. Given an **`Attestation` object**, the object's own
Ed25519 signature is always checked — see the version note below, because 0.8.0
did **not** — and `is_offline=False` adds a server lookup on top of it.
...
<Aside type="caution" title="Offline `verify()` became a signature check in 0.8.1">
  **0.8.1 (unpublished).** The offline branch rebuilds the signed payload from
  the receipt's own fields (`glacis.crypto.offline_signed_payload`) and verifies
  the signature against it with `Ed25519Runtime.verify()`, under the public key
  **on the receipt**. It needs no signing seed, so it works on receipts you did
  not sign. Alter any signed field — `service_id`, `evidence_hash`, the
  timestamp, the `control_plane_results` content — and it returns
  `valid=False`. `glacis.verify.verify_offline()` is that implementation, and
  `python -m glacis verify` calls the same one.

  `error` names which failure it was:

  | Prefix | Meaning |
  | --- | --- |
  | `signature_invalid` | The payload rebuilt and the signature does not verify over it. |
  | `structural` | The receipt could not be turned into verifiable form at all — undecodable `public_key` or `signature`, or no `timestamp`. |
  | `cpr_unrecoverable` | The store could not return the signed `control_plane_results`, so the payload cannot be rebuilt and no verdict is possible. |
  | `cpr_hash_mismatch` / `cpr_hash_orphaned` | A named degradation of the **unsigned** `cpr_hash`. It can accompany `valid=True`: the signature is the authority, not the hash. |

  **0.8.0, which is what `pip install glacis` gives you, checks none of this.**
  It derived a public key from the client's own `signing_seed` and compared it
  to `attestation.public_key`; if they matched it returned `valid=True` without
  looking at `signature` at all, and if the client had no seed `signature_valid`
  was hardcoded `True`. Both behaviours — the old one and the new one — are
  pinned by `docs/scripts/verify-doc-snippets.py`.

  The PyNaCl routine in
  [Verify from Python](/verify/cli/#verify-a-receipt-yourself) is the check
  written out longhand, and it works against any version.
</Aside>

<Aside type="caution" title="An unsigned field chose the verifier until 0.8.1.dev0">
  `is_offline` and `id` are not inside the signature. Until 0.8.1.dev0 they
  chose the verifier outright: setting `is_offline=False` and `id` to a valid
  online attestation routed `verify()` to a server lookup, and the object you
  passed in was never examined. The server answered about the id and the caller
  read `valid=True` as an answer about their bytes. The CLI reclassified the
  same way.

  Fixed in this repository, staged as 0.8.1 and unpublished. `is_offline` now
  selects **additional** verification, never a bypass:

  - The offline signature check runs on every supplied object, always.
  - `is_offline=False` also looks up `receipt.id`. That answer is about an id,
    so it is applied to your object only if the two **bind** — matching
    `signature` and `evidence_hash`, plus `service_id` and `operation_type`
    when both carry them.
  - **Bound, and the object's own signature verified:** you get the server's
    [`VerifyResult`](#verifyresult), with `error` naming the binding and what
    the log entry carries nothing about (`control_plane_results`, `cpr_hash`,
    `evidence`, `review`, and the rest).
  - **Bound, but the object's own check failed:** fail closed. Binding
    compares the *strings* of `signature` and `evidence_hash` — a string-equal
    signature is not a verified one. You get the failed
    [`OfflineVerifyResult`](#offlineverifyresult) with `error` carrying
    `bound-but-unverified:`; the server's `valid` is never applied to bytes
    that failed their own Ed25519 check.
  - **Unbound:** none of the server's answer is returned — not the org, not the
    proof, not the tree head. You get the object's own
    [`OfflineVerifyResult`](#offlineverifyresult), `valid` reflecting the bytes
    you passed, and `error` saying the lookup happened and was not applied.

  So a receipt whose signature does not verify is `valid=False` whatever the
  server says about the id it claims.
</Aside>
...
**Returns:** [`VerifyResult`](#verifyresult) — the log entry's verdict, for an id looked up directly or an object bound to its entry — or [`OfflineVerifyResult`](#offlineverifyresult), the supplied object's own signature check
...
Hash a payload using `glacis.crypto` canonical JSON + SHA-256. Reproducible across runtimes for ASCII strings and integers; see the [divergences from RFC 8785](/connect/quickstart/#the-canonicalisation-actually-used) before relying on cross-language agreement for anything else.
...
| Method | Description |
|--------|-------------|
| `hash(payload)` | `glacis.crypto` canonical JSON + SHA-256. Returns `str`. |
...
class Attestation(BaseModel):
    ...
    cpr_recovery_error: Optional[str]   # 0.8.1, unpublished
...
| `control_plane_results` | `Optional[dict[str, Any]]` | Control plane results (dict on wire). **Inside the offline signature**, and transmitted verbatim in online mode |
| `evidence` | `Optional[Evidence]` | L1 sampled evidence payload. On an online L1/L2 attestation this holds the **raw** `input` and `output` you passed — check before serialising a receipt you intend to share |
...
| `cpr_recovery_error` | `Optional[str]` | 0.8.1 (unpublished). Set only by storage reconstruction, when a stored receipt carries a `cpr_hash` but the signed `control_plane_results` could not be returned. The signed payload cannot be rebuilt, so `verify()` fails closed with this string as its `error`. Never signed, never transmitted |
...
| `error` | `Optional[str]` | Why it failed, prefixed with the failure's name. Also set — on a result that **passed** — when you verified an `Attestation` object rather than an id, to name the binding: `bound:` says the object matches this log entry on `signature` and `evidence_hash`, and lists what the entry carries nothing about |
...
| Field | Type | Description |
|-------|------|-------------|
| `valid` | `bool` | Mirrors `signature_valid` |
| `witness_status` | `Literal["UNVERIFIED"]` | Always `"UNVERIFIED"` for offline attestations |
| `signature_valid` | `bool` | A real Ed25519 check as of 0.8.1; **not** one in 0.8.0 — see the version note under [`verify()`](#glacisverify) |
| `attestation` | `Optional[Attestation]` | The verified attestation |
| `error` | `Optional[str]` | Set when verification failed, **or** carries the advisory `cpr_hash_mismatch` on a receipt whose `valid` follows the signature. `cpr_hash` is unsigned and the signature covers the control-plane *content*, so a hash that disagrees with it is reported by name alongside `valid=True` rather than deciding it. Also carries `unbound:` when you passed an `Attestation` object with `is_offline=False` whose id was looked up and did not describe it, or `bound-but-unverified:` when the id's log entry matched the object's binding fields but the object's own signature check failed — `valid` follows that check, never the lookup. See [`verify()`](#glacisverify) |
...
Hash a payload using `glacis.crypto` canonical JSON + SHA-256. This is the primary hashing function for the transparency log. It reproduces across Python, TypeScript and Rust for ASCII strings and integers, but **not** in general -- see [the canonicalisation actually used](/connect/quickstart/#the-canonicalisation-actually-used).
...
Serialize data to the SDK's canonical JSON: keys sorted by Unicode code point, no whitespace, values rendered by Python's `json.dumps`. The module docstring calls this RFC 8785; it diverges for non-ASCII strings, several float forms and supplementary-plane keys.
...
<Aside type="caution">
  Canonicalisation makes key order irrelevant, so the same logical JSON object hashes the same way *within* this implementation. It does **not** guarantee agreement with a strict RFC 8785 implementation in another language: escaping of non-ASCII text, the rendering of whole-number floats and very large magnitudes, and the ordering of supplementary-plane keys all differ. [The exact list](/connect/quickstart/#the-canonicalisation-actually-used).
</Aside>
