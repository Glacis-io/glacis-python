...
Verify an attestation. For online attestations, calls the server API. For offline
attestations, verifies the Ed25519 signature locally — see the version note below,
because 0.8.0 did **not**.
...
| `error` | `Optional[str]` | Why it failed, prefixed with the failure's name. Also set on a receipt that **passed** when an unsigned field disagrees with the signed content (`cpr_hash_mismatch`) |
