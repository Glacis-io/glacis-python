...
<Aside type="caution" title="An unsigned field chose the verifier until 0.8.1.dev0">
  ...
  - **Bound:** you get the server's [`VerifyResult`](#verifyresult), with
    `error` naming the binding and what the log entry carries nothing about
    (`control_plane_results`, `cpr_hash`, `evidence`, `review`, and the rest).
