...
Attach business-specific metadata to every attestation created by the client. Custom metadata is merged with provider defaults (`provider` and `model`, which are set automatically).
...
# {"provider": "openai", "model": "gpt-4o", "department": "legal", "use_case": "contract-review"}
```
...
<Aside type="note">
  Streaming is **not yet supported**. Passing `stream=True` to `client.chat.completions.create()` may produce errors or incomplete attestations, as the wrapper expects a non-streaming response object. Streaming support is planned for a future release.
</Aside>
