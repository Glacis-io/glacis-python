...
Sampling decisions are deterministic and auditor-reproducible. Given the same `evidence_hash` + `signing_seed` + sampling rates, the SDK always produces the same decision.

The algorithm:

1. Compute an HMAC-SHA256 tag over the `evidence_hash` using the `signing_seed` as key
2. Extract the first 8 bytes as a big-endian `uint64` value (`sample_value`)
3. Compare `sample_value` against thresholds derived from `l1_rate` and `l2_rate`

This means an auditor with access to the signing seed can independently verify that sampling decisions were correctly applied.
...
<Aside type="note">
  `should_review()` requires offline mode (it uses the `signing_seed` for HMAC computation).
</Aside>
