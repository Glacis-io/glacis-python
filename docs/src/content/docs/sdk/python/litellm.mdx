...
Use `get_last_receipt()` to retrieve the attestation from the most recent API call. Receipts are stored using `contextvars.ContextVar`, so each thread and each async task maintains its own last receipt independently.
...
<Aside type="caution">
  `get_last_receipt()` returns the receipt for the **current context** (thread or async task). In concurrent applications, each thread and each `asyncio.Task` tracks its own last receipt. Call `get_last_receipt()` immediately after the API call to avoid it being overwritten by a subsequent call.
</Aside>
